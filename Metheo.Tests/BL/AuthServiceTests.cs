...
public class AuthServiceTests
{
    ...
    public AuthServiceTests()
    {
        ...
        _authService = new AuthService(_authDataAccessMock.Object, _tokenServiceMock.Object,
    ...
    public async Task LoginAsync_ValidCredentials_ReturnsToken()
    {
        ...
        {
            id = 1, email = "testuser@example.com", password = BCrypt.Net.BCrypt.HashPassword("password123"),
            role_name = "Admin", permission_name = "Read"
        };

        _authDataAccessMock.Setup(x => x.GetUserByEmailAsync(loginRequest.Email)).ReturnsAsync(userLoginResult);
        _passwordServiceMock.Setup(x => x.VerifyPassword(loginRequest.Password, userLoginResult.password))
            .Returns(true);
    ...
    public async Task LoginAsync_InvalidCredentials_ReturnsNull()
    {
        ...
        var userLoginResult = new UserLoginResult
        {
            id = 1, email = "testuser@example.com", password = BCrypt.Net.BCrypt.HashPassword("differentPassword"),
            role_name = "Admin", permission_name = "Read"
        };
